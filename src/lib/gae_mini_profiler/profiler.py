...
class SharedStatsHandler(RequestHandler):
    ...
    def get(self):
        ...
        import templatetags
        profiler_includes = templatetags.profiler_includes_request_id(
            request_id, True)
        ...
        for request_id in list_request_ids:
            ...
            if request_stats and not request_stats.disabled:
                ...
                for property in RequestStats.serialized_properties:
                    dict_request_stats[
                        property] = request_stats.__getattribute__(property)
...
class RequestStats(object):
    ...
    def get(request_id):
        if request_id:
            ...
            compressed_pickled = memcache.get(
                RequestStats.memcache_key(request_id))
...
class RequestProfiler(object):
    ...
    def profile_start_response(self, app, environ, start_response):
        ...
        if self.mode == Mode.SIMPLE:
            ...
            # If we're dealing w/ a generator, profile all of the .next calls
            # as well
...
class ProfilerWSGIMiddleware(object):
    ...
    def __call__(self, environ, start_response):
        ...
        # Never profile calls to the profiler itself to avoid endless
        # recursion.
        if (not config.should_profile() or
            ...
            def profiled_start_response(status, headers, exc_info=None):
                ...
                # Append headers used when displaying profiler results from
                # ajax requests
    ...
    def headers_with_modified_redirect(environ, headers):
        ...
        for header in headers:
            if header[0] == "Location":
                ...
                if match:
                    request_id_chain = ",".join(
                        [match.groups()[0], request_id_chain])
