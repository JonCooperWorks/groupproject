...
def do_form_field(parser, token):
    ...
    if len(parts) < 2:
        raise template.TemplateSyntaxError(
            '%r tag must have the form field name as the first value, followed by optional key="value" attributes.' % parts[0])
    ...
    if len(parts) == 3:
        ...
        if (len(raw_args) % 2) != 0:
            raise template.TemplateSyntaxError(
                '%r tag received the incorrect number of key=value arguments.' % parts[0])
